...
contract Identity {
    ...
    function humanVerify(
        string memory _adhaar,
        uint256 _today // _today is seconds in today's date
    ) public {
    ...
    function generateAPIKeyForHost(
        string memory _adhaar,
        string memory _name,
        string memory _key // host's adhaar needed
    ) public {
        require(
            compareStrings(wallet[msg.sender], _adhaar),
            "The host must be verified!"
        );
        require(!deactivated[_adhaar], "Adhaar has been deactivated");
        require(!deactivatedHost[_key], "API has already been deactivated");
    ...
    function deactivate(string memory _adhaar) public {
        require(
            compareStrings(wallet[msg.sender], _adhaar),
            "Address does not belong to adhaar holder"
        );
        require(!deactivated[_adhaar], "Adhaar has been deactivated");
    ...
    function registerSecurityQuestions(
        string memory _adhaar,
    ...
    function returnSecurityQuestions(
        string memory _adhaar
    ) public view returns (SecurityQuestions memory) {
        require(compareStrings(wallet[msg.sender], _adhaar));
        require(!deactivated[_adhaar], "Adhaar has been deactivated");
    ...
    function identityExists(string memory _adhaar) public view returns (bool) {
        if (deactivated[_adhaar]) return false;

        if (bytes(wallet[msg.sender]).length > 0) return true;

        if (adhaar[_adhaar].dateOfBirth > 0) return true;
